...
The backend/api/error_responses.py file provides error response information according to [RFC 7807](https://tools.ietf.org/html/rfc7807) for error responses. The response body will contain a JSON object with the following fields:
...
| --------- | --------------------------- | ----------------------------------------------------------------------------------------------------------------------- |
| 401 (authentication_failure)       | authentication_failure      | Request has an invalid API Key or other authentication mechanism                                                        |
| 400 (duplicate_resource)       | duplicate_resource          | You attempted to create a resource that either already exists exactlky as is, or violates db constraints.               |
| 404 (resource_not_found)       | resource_not_found          | The requested resource does not exist.                                                                                  |
| 400       | external_connection_invalid (400) | You specified a connection with an outside resource (like Stripe) that is invalid.                                      |
| 400       | invalid_state (400)               | The requested action is not allowed in the current state of the resource. This is usually a busienss logic restriciton. |
| 400       | invalid_request (400)             | The request is invalid. Check the `detail` for more information.                                                        |
...
| HTTP Code | Error Title                | Description                                                                                                                                                                                                           |
| --------- | -------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 500 (aggregation_engine_failure) (external_connection_failed) (server_error)       | server_error               | Something went wrong on our end. We're looking into it!                                                                                                                                                               |
